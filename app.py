...
import os
import razorpay
RAZORPAY_KEY_ID='rzp_test_p1Y8tik4gQ37wZ'
RAZORPAY_KEY_SECRET='vjiqjfyj14fUsizxWatlpiRZ'
...
def add_to_cart(itemid,name,category,price,quantity):
...
def addedsuccess():
    return render_template('addedsucces.html')
...
def success():
    if session.get('user'):
        ...
        try:
            ...
            cursor=mydb.cursor(buffered=True)
            cursor.execute('insert into orders(itemid,item_name,total_price,user,qty) values(%s,%s,%s,%s,%s)',[itemid,name,total_price,session.get('user'),qty])
            mydb.commit()
            cursor.close()
...
def admin_dashboard():
    if session.get('admin'):
        ...
        cursor.execute('SELECT COUNT(*) FROM additems')
        total_items = cursor.fetchone()[0]
        
        cursor.close()
        
        return render_template('admin_dashboard.html', total_orders=total_orders, total_users=total_users, total_items=total_items)
    else:
        return redirect(url_for('adminlogin'))
